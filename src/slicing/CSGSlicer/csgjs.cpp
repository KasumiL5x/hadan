...
struct csgjs_vertex
{
	...
	csgjs_vertex( csgjs_vector thePos )
		: pos(thePos)
	{
	}
};
...
//// NEW
//struct csgjs_model_extended
//{
//	std::vector<csgjs_vertex> vertices;
//	std::vector<int> indices; // indices following face counts
//	std::vector<int> counts; // defines face counts e.g. [3,4] means 3 indices followed by 4 indices; two separate faces
//};
...
//csgjs_model_extended csgjs_intersection_extended( const csgjs_model_extended& a, const csgjs_model_extended& b );
...
//inline static std::vector<csgjs_polygon> csgjs_modelToPolygons_extended( const csgjs_model_extended& model )
//{
//	std::vector<csgjs_polygon> list;
//	unsigned int offset = 0;
//	for( size_t i = 0; i < model.counts.size(); ++i )
//	{
//		std::vector<csgjs_vertex> poly;
//		for( size_t j = 0; j < model.counts[i]; ++j )
//		{
//			const int idx = model.indices[offset++];
//			const csgjs_vertex& vtx = model.vertices[idx];
//			poly.push_back(vtx);
//		}
//		list.push_back(csgjs_polygon(poly));
//	}
//	return list;
//}
...
//inline static csgjs_model_extended csgjs_modelFromPolygons_extended( const std::vector<csgjs_polygon>& polygons )
//{
//	csgjs_model_extended model;
//	int p = 0;
//	for( size_t i = 0; i < polygons.size(); ++i )
//	{
//		const csgjs_polygon& poly = polygons[i];
...
//		}
//		model.counts.push_back(poly.vertices.size());
//	}
//	return model;
//}
...
//inline static csgjs_model_extended csgjs_operation_extended( const csgjs_model_extended& a, const csgjs_model_extended& b, csg_function fun )
//{
//	csgjs_csgnode * A = new csgjs_csgnode(csgjs_modelToPolygons_extended(a));
//	csgjs_csgnode * B = new csgjs_csgnode(csgjs_modelToPolygons_extended(b));
//	csgjs_csgnode * AB = fun(A, B);
//	std::vector<csgjs_polygon> polygons = AB->allPolygons();
//	delete A; A = 0;
//	delete B; B = 0;
//	delete AB; AB = 0;
//	return csgjs_modelFromPolygons_extended(polygons);
//}
...
//csgjs_model_extended csgjs_intersection_extended( const csgjs_model_extended& a, const csgjs_model_extended& b )
//{
//	return csgjs_operation_extended(a, b, csg_intersect);
//}
